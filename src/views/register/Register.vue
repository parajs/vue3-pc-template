                    ...
                    <el-form  :model="form" :rules="rules" ref="ruleForm" class="text-center">
                        ...
                        <el-form-item prop="code" class="mt-8">
                         ...
                         <el-form-item prop="password" class="mt-8">
                            <el-input size="large" v-model="form.password" placeholder="请输入密码6-18英文、数字、符号的组合"></el-input>
                        </el-form-item>
                        <el-form-item class="mt-8">
                            <el-button size="large" style="width:100%" type="primary" round @click="onSubmit">注册</el-button>
                        </el-form-item>
                    </el-form>
                </el-card>
            </el-col>
...
const getVertifyCode = ()=>{
    if(!isSending.value){
        ...
        if(reg.test(form.email)){
            ...
            const { data, error, isFinished } =  codeGet({email: form.email,type: 'register'})
                watch(isFinished,()=>{
...
const onSubmit = ()=>{
    ruleForm.value.validate((valid: boolean) => {
          if (valid) {
            ...
            watch(data,()=>{
                ElMessage.info("注册成功");
                store.dispatch("user/loginPassword",{username: form.email, password: form.password}).then(()=>{
